...
parser.set_defaults(fun=cli.handle_missing_args, start_daemon=False, parser=parser)
subparsers = parser.add_subparsers()

# copy
parser_copy = subparsers.add_parser("copy", help="copy a file")
parser_copy.set_defaults(fun=cli.handle_copy_move, mode="copy", start_daemon=True)

# move
parser_move = subparsers.add_parser("move", help="move a file")
parser_move.set_defaults(fun=cli.handle_copy_move, mode="move", start_daemon=True)

# convert
parser_convert = subparsers.add_parser("convert", help="convert an audio file")
parser_convert.set_defaults(fun=cli.handle_convert, start_daemon=True, mode="convert")
...
parser_re.set_defaults(fun=cli.handle_re, start_daemon=True)
...
parser_echo.set_defaults(fun=cli.handle_echo, start_daemon=True)

# progress
parser_progress = subparsers.add_parser("progress", help="show progress bar")
parser_progress.set_defaults(fun=cli.handle_queue_progress, start_daemon=True)

# queue management
parser_queue = subparsers.add_parser("queue", help="manage the file processing queue (start, stop, ...)")
parser_queue.set_defaults(start_daemon=True, fun=cli.handle_missing_args, parser=parser_queue)
parser_queue_sub = parser_queue.add_subparsers()
parser_queue_sub.add_parser("start", help="start processing the queue").set_defaults(fun=cli.handle_simple_command, command=Command.QUEUE_START)
parser_queue_sub.add_parser("stop",  help="stop processing the queue").set_defaults(fun=cli.handle_simple_command, command=Command.QUEUE_STOP)
parser_queue_sub.add_parser("flush", help="completely reset the queue (including finished, failed and skipped tasks)").set_defaults(fun=cli.handle_simple_command, command=Command.QUEUE_FLUSH_ALL)
parser_queue_sub.add_parser("flush-pending", help="remove all pending tasks from the queue").set_defaults(fun=cli.handle_simple_command, command=Command.QUEUE_FLUSH_PENDING)
parser_queue_sub.add_parser("progress", help="show interactive progress bar").set_defaults(fun=cli.handle_queue_progress)
parser_queue_sub.add_parser("active", help="show number of active queues (usually just one)").set_defaults(fun=cli.handle_simple_command, command=Command.QUEUE_ACTIVE_PROCESSES)
parser_queue_sub.add_parser("is-active", help="show number of active queues (usually just one)").set_defaults(fun=cli.handle_simple_command, command=Command.QUEUE_IS_ACTIVE)
parser_queue_sub.add_parser("show", help="show the queue").set_defaults(fun=cli.handle_simple_command, command=Command.QUEUE_SHOW)

# daemon management
parser_daemon = subparsers.add_parser("daemon", help="manage the daemon process")
parser_daemon.set_defaults(start_daemon=False, fun=cli.handle_missing_args, parser=parser_daemon)
parser_daemon_sub = parser_daemon.add_subparsers()
parser_daemon_sub.add_parser("restart", help="restart the daemon").set_defaults(fun=cli.handle_daemon_restart)
parser_daemon_sub.add_parser("stop", help="stop the daemon").set_defaults(fun=cli.handle_daemon_stop)
parser_daemon_sub.add_parser("is-active", help="check if daemon is alive").set_defaults(fun=cli.handle_daemon_is_active)
parser_daemon_sub.add_parser("destroy", help="immediately terminate the daemon and empty the queue").set_defaults(fun=cli.handle_daemon_destroy, start_daemon=True)
parser_daemon_sub.add_parser("facts", help="return information about the daemon").set_defaults(fun=cli.handle_simple_command, command = Command.DAEMON_FACTS, start_daemon=False)
...
if args.start_daemon:
    cli.handle_daemon_start(args, client)
    cli.wait_for_daemon(client, timeout=10)
